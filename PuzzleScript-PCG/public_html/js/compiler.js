...
{
	// console.log("processRuleString\n\tline=%s\n\tstate=%o\n\tlineNumber=%s\n\tcurRules=%o", line, state, lineNumber, curRules);
/*
...
function rulesToArray(state) {
	// console.log('rulesToArray(), state=%o', state);
	_rules = [];

	var oldrules = state.rules;
	var rules = [];
	var loops=[];
	state.ruleLineNumbers = [];
	...
	for (var i = 0; i < oldrules.length; i++) {
		...
		rules.push(newrule);
		_rules.push(newrule);
		state.ruleLineNumbers.push(lineNumber);
	}
	...
	for (var i=0;i<rules3.length;i++) {
        ...
        /*var _rules4 = [];
        for (var i = 0; i < state.originalRules.length; i++) {
            var rule=state.originalRules[i];
            var tempRules = concretizePropertyRule(state, rule,rule.lineNumber);
            for(var j = 0; j < tempRules.length; j++){
                delete tempRules[j].direction;
                tempRules[j].directions = rule.directions;
            }
            _rules4 = _rules4.concat(tempRules);
        }*/

	state.rules = rules4;
        //state.originalRules = _rules4;
}
