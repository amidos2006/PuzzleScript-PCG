...
function update() {
    ...
    if (quittingTitleScreen) {
        if (skipMessages || timer/1000>0.3) {
            quittingTitleScreen=false;
            nextLevel();
        }
    }
    if (againing) {
    	if (skipMessages || timer>againinterval) {
    		againing=false;
    		processInput(-1);
    	}
    }
    if (quittingMessageScreen) {
        if (skipMessages || timer/1000>0.15) {
    ...
    if (winning) {
        if (skipMessages || timer/1000>0.5) {
            winning=false;
            nextLevel();
        }
    }
    if (keybuffer.length>0) {
	    ...
	    if (skipMessages || keyRepeatTimer>repeatinterval/(Math.sqrt(keybuffer.length))) {
	    	keyRepeatTimer=0;
    ...
    if (autotickinterval>0&&!textMode) {
        autotick+=deltatime;
        if (skipMessages || autotick>autotickinterval) {
            autotick=0;
            autoTickGame();
        }
    }
